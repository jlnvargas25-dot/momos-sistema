    ...
    <div className="mt-7 mb-6 rounded-[28px] overflow-hidden border shadow-sm" style={{ borderColor: "#D9C2AE", background: "linear-gradient(145deg,#FFF,#FFF9F2)" }}>
      <div className="px-4 sm:px-5 py-4 border-b flex flex-col lg:flex-row lg:items-center justify-between gap-3" style={{ borderColor: T.border, background: "linear-gradient(135deg,#FFF3EA,#F9E7DE)" }}>
        ...
        <div className="flex flex-wrap gap-2">
          {[{ id: "Biblioteca", label: "Biblioteca" }, { id: "Activos de producción", label: "Preparar referencias" }, { id: "Estudio", label: "Crear" }, { id: "Producción", label: "Trabajos creativos" }, { id: "Integraciones", label: "Conexiones" }].map((item) => <button key={item.id} type="button" onClick={() => setSection(item.id)} className="rounded-full border px-3 py-2 text-[11px] font-extrabold" style={{ borderColor: section === item.id ? T.coral : T.border, background: section === item.id ? T.coral : "#fff", color: section === item.id ? "#fff" : T.choco }}>{item.label}</button>)}
        </div>
      </div>
