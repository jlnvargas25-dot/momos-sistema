...
function ProductionVisualScopeFields({ form, onChange, enabled }) {
  ...
  return <div className="rounded-2xl border p-3 mt-3" style={{ borderColor: "#C8B3D9", background: "#fff" }}>
    ...
    <label className="flex gap-2 items-start rounded-xl px-2.5 py-2 mb-2 text-xs font-extrabold" style={{ background: form.sourceQuality === "Original con escarcha" ? "#FFF2D8" : "#F4FAF2" }}><input type="checkbox" className="mt-0.5" disabled={form.sourceQuality === "Original con escarcha"} checked={form.sourceQuality !== "Original con escarcha" && Boolean(form.canonical)} onChange={(event) => onChange({ ...form, canonical: event.target.checked })} /><span>Declarar como máster canónico<span className="block text-[9px] font-normal mt-0.5" style={{ color: T.choco2 }}>{form.sourceQuality === "Original con escarcha" ? "La escarcha se conserva como variante artística y nunca puede ser el máster." : "Solo Administración puede declarar la referencia oficial para IA."}</span></span></label>
...
function VisualQualityReviewFields({ form, onChange }) {
  ...
  return <div className="rounded-2xl border p-3 mt-3" style={{ borderColor: "#B8D3B2", background: "#F4FAF2" }}>
    ...
    <div className="text-[10px] mt-1 mb-3" style={{ color: T.choco2 }}>Marcá los seis controles revisados. Escarcha o condensación puede conservarse como variante artística, pero bloquea imagen, video y Element hasta tener un máster limpio.</div>
    ...
    <Field label="Conclusión de la revisión"><textarea className={inputCls} style={inputStyle} rows="3" value={form.qualityReviewNotes || ""} onChange={(event) => onChange({ ...form, qualityReviewNotes: event.target.value })} placeholder="Ej. Max tiene escarcha que tapa la textura; repetir con luz suave y fondo limpio." /></Field>
  </div>;
}
...
function AgencyBrandStudio({ db, user, refrescar, initialIntent = null, onIdentityChanged }) {
  ...
  const visualCleanMasterReady = Boolean(db.visualCleanMasterReady);
  ...
  function openImprovedAssetUpload(asset) {
    ...
    setAssetForm({
      ...
      productionEnabled: productionAssetsReady, ...profile, sourceQuality: "Restaurado",
      variantLabel: "Derivado limpio", qaStatus: "Pendiente", canonical: false,
  ...
  async function saveAssetMetadata() {
    ...
    try {
      ...
      if (assetEditForm.qualityReviewEnabled) {
        if (!visualQualityReady) throw new Error("La información general se guardó, pero falta aplicar H110 para revisar calidad de IA.");
      ...
      toast("ok", assetEditForm.qualityReviewEnabled
        ? "Información y revisión maestra de calidad guardadas; el original permaneció intacto"
  ...
  return (
    <div className="mt-7 mb-6 rounded-[28px] overflow-hidden border shadow-sm" style={{ borderColor: "#D9C2AE", background: "linear-gradient(145deg,#FFF,#FFF9F2)" }}>
      ...
      {section === "Biblioteca" ? <div className="p-4 sm:p-5">
        ...
        {visualCleanMasterReady && <div className="rounded-3xl border p-4 mb-4" style={{ borderColor: "#B8D3B2", background: "#F4FAF2" }}><div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div><div className="text-[9px] uppercase tracking-[.14em] font-extrabold" style={{ color: "#315B35" }}>Protocolo anti-escarcha</div><div className="font-extrabold">Toma limpia primero; variantes artísticas después</div><div className="text-[10px] mt-1" style={{ color: T.choco2 }}>El original nunca se modifica. Escarcha y condensación pueden conservarse para contenido, pero imagen IA, video y Elements solo reciben un máster limpio, canónico y con linaje.</div></div><div className="flex gap-1.5 text-[9px] font-extrabold"><span className="rounded-full px-2 py-1 bg-white">1 · Capturar</span><span className="rounded-full px-2 py-1 bg-white">2 · Revisar</span><span className="rounded-full px-2 py-1 bg-white">3 · Derivar</span></div></div></div>}
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-2 mb-5">
          {[["Clasificados",productionLibrary.summary.profiled],["QA aprobado",productionLibrary.summary.approved],["Másteres limpios",productionLibrary.summary.cleanMasters],["Variantes artísticas",productionLibrary.summary.artisticFrost],["Pendientes de máster",productionLibrary.summary.cleanMasterPending],["Imagen IA",productionLibrary.summary.imageReady],["Video IA",productionLibrary.summary.videoReady],["Elements",productionLibrary.summary.elementReady],["Nueva toma",productionLibrary.summary.needsNewCapture],["Manos / UGC",productionLibrary.summary.humanComponents],["Locaciones",productionLibrary.summary.locations],["Ángulos",productionLibrary.summary.multiviewAngles],["Sets visuales",productionLibrary.summary.visualSets],["Frente + atrás",productionLibrary.summary.frontBackSets],["Packs aprobados",productionLibrary.summary.approvedPacks]].map(([label,value]) => <div key={label} className="rounded-2xl border p-3" style={{ borderColor: T.border, background: T.soft }}><div className="text-[9px] uppercase tracking-wider font-extrabold" style={{ color: T.choco2 }}>{label}</div><div className="display text-2xl font-semibold" style={{ color: T.coral }}>{value}</div></div>)}
        </div>
        {visualLibraryReady && productionLibrary.visualSets.length > 0 && <div className="mb-5"><div className="text-[9px] uppercase tracking-[.14em] font-extrabold mb-2" style={{ color: T.coral }}>Sets multivista reutilizables</div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">{productionLibrary.visualSets.map((set) => <div key={set.key} className="rounded-2xl border p-3" style={{ borderColor: set.videoReadyAssets > 1 ? "#B8D3B2" : T.border, background: set.videoReadyAssets > 1 ? "#F4FAF2" : "#FFF9F2" }}><div className="flex justify-between gap-2"><span className="font-extrabold text-xs">{set.key}</span><span className="text-[9px] font-extrabold">{set.assets.length} activos</span></div><div className="text-[9px] mt-1" style={{ color: T.choco2 }}>{set.views.join(" · ") || "Sin vistas"}</div><div className="text-[9px] font-bold mt-2" style={{ color: set.videoReadyAssets > 1 ? "#315B35" : "#7A5410" }}>{visualQualityReady ? `${set.videoReadyAssets} aptos para video · ${set.elementReadyAssets} aptos para Element` : set.hasFrontAndBack ? "✓ Frente y vista trasera cubiertos" : "Falta completar frente o vista trasera"}</div></div>)}</div></div>}
        ...
        return <Modal title={assetEditForm ? `Editar · ${detailAsset.name}` : detailAsset.name} onClose={() => { setDetailAssetId(null); setAssetEditForm(null); }} extraWide topLayer>
          <div className="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(340px,.75fr)] gap-5">
            ...
            {!assetEditForm ? <div>
              ...
              <div className="flex flex-wrap gap-2">{canWrite && <Btn onClick={() => beginAssetMetadataEdit(detailAsset)}>Editar información</Btn>}{canWrite && visualQualityReady && ["Foto","Logo"].includes(detailAsset.mediaType) && <Btn kind="soft" onClick={() => { openImprovedAssetUpload(detailAsset); setDetailAssetId(null); setAssetEditForm(null); }}>Subir versión mejorada</Btn>}{detailDeletion.allowed && <Btn kind="ghost" onClick={() => openDeleteConfirmation(detailAsset)}>{isOfficialBrandLogo(detailAsset) ? "Eliminar logo" : "Eliminar definitivamente"}</Btn>}<Btn kind="ghost" onClick={() => { setDetailAssetId(null); setAssetEditForm(null); }}>Cerrar</Btn></div>
            </div> : <div>
              ...
              {visualQualityReady && assetEditForm.productionEnabled && <div className="rounded-3xl border p-3 mb-3" style={{ borderColor: "#B8D3B2", background: "#F4FAF2" }}><label className="flex gap-2 items-start text-sm font-extrabold"><input type="checkbox" className="mt-1" checked={Boolean(assetEditForm.qualityReviewEnabled)} onChange={(event) => setAssetEditForm({ ...assetEditForm, qualityReviewEnabled: event.target.checked, qualityIssues: [], qualityChecksCompleted: [], qualityReviewNotes: "" })} /><span>Registrar una nueva revisión de calidad IA<span className="block text-[10px] font-normal" style={{ color: T.choco2 }}>Crea evidencia versionada; no modifica ni reemplaza el archivo original.</span></span></label>{assetEditForm.qualityReviewEnabled && <VisualQualityReviewFields form={assetEditForm} onChange={setAssetEditForm} />}</div>}
