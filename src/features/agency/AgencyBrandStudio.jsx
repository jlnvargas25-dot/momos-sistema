  ...
  return (
    <div className="mt-7 mb-6 rounded-[28px] overflow-hidden border shadow-sm" style={{ borderColor: "#D9C2AE", background: "linear-gradient(145deg,#FFF,#FFF9F2)" }}>
      ...
      {section === "Biblioteca" ? <div className="p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-8 gap-2 mb-5">
          {[["Clasificados",productionLibrary.summary.profiled],["QA aprobado",productionLibrary.summary.approved],["Manos / UGC",productionLibrary.summary.humanComponents],["Locaciones",productionLibrary.summary.locations],["Ángulos",productionLibrary.summary.multiviewAngles],["Sets visuales",productionLibrary.summary.visualSets],["Frente + atrás",productionLibrary.summary.frontBackSets],["Packs aprobados",productionLibrary.summary.approvedPacks]].map(([label,value]) => <div key={label} className="rounded-2xl border p-3" style={{ borderColor: T.border, background: T.soft }}><div className="text-[9px] uppercase tracking-wider font-extrabold" style={{ color: T.choco2 }}>{label}</div><div className="display text-2xl font-semibold" style={{ color: T.coral }}>{value}</div></div>)}
        </div>
