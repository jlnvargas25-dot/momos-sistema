...
function AgenciaControl({ db: sourceDb, user, refrescar, go }) {
  ...
  const advancedModules = {
    ...
    identity: [
      ...
      { id: "visual-quality", icon: "🎬", eyebrow: "Calidad para IA", title: "Activos listos para crear", description: "Verificá qué fotos sirven para imagen, video y Elements, y cuáles necesitan una nueva toma.", metric: visualProductionLibrary.summary.videoReady, metricLabel: "aptos para video", tone: visualQualityPending > 0 ? "gold" : "green", ready: db.visualQualityReady === true },
    ],
    strategy: [
      { id: "strategy-humanization", icon: "♡", eyebrow: "Conexión de marca", title: "Humanización y Comunidad", description: "Series, episodios y señales reales sin testimonios inventados ni datos personales.", metric: Number(db.agencyHumanization?.summary?.approved_series || 0), metricLabel: "series activas", tone: "rose", ready: db.agencyHumanizationReady === true },
    ...
    results: [
      { id: "results-formulas", icon: "✦", eyebrow: "Memoria creativa", title: "Fórmulas ganadoras", description: "Versioná lo que funciona y compará Meta/TikTok con ventas y margen reales.", metric: Number(db.agencyCreativeIntelligence?.summary?.winners || 0), metricLabel: "ganadoras", tone: "coral", ready: db.agencyCreativeIntelligenceReady === true },
  ...
  return (
    <section className="mb-6" aria-label="Agencia Comercial MOMOS" data-modern-projection={JSON.stringify(db.agencyModernProjectionStatus || {})}>
      <div className="rounded-2xl overflow-hidden border shadow-sm" style={{ borderColor: T.border, background: T.surface }}>
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mb-5" aria-label={`Herramientas de ${activeAdvancedArea.label}`}>
            {activeAdvancedModules.map((module) => <AgencyAdvancedModuleCard key={module.id} {...module} status={module.ready === false ? "No disponible" : module.metric > 0 ? "Con información" : "Listo para usar"} onOpen={() => { if (module.id === "creative-library") openBrandLibrary(); else if (module.id === "visual-quality") openBrandLibrary({ section: "Activos de producción" }); else setAdvancedDetail(module.id); }} />)}
          </div>
