...
test("Agencia usa su snapshot cerrado sin rehidratar ni reemplazar catálogos", () => {
  ...
  assert.match(readModelSource, /supabase\.rpc\("momos_agency_snapshots_v1"\)/,
    "la ruta normal debe cargar los cuatro scopes con un solo RPC");
...
test("Realtime de Agencia usa un solo outbox y nunca las tablas crudas", () => {
  assert.match(mainSource, /const realtimeDomains = new Set\(syncDomainsForView\(vista\)\)/);
  ...
  assert.match(mainSource, /perfil\?\.id, vista, Boolean\(db\?\.operationalControlReady\), Boolean\(db\?\.crmServerReady\), Boolean\(db\?\.agencySnapshotReady\)/,
...
test("pre-H66 mantiene polling de Agencia aunque Operaciones tenga Realtime", () => {
